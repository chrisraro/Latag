...
/** Case- and diacritic-insensitive key — mirrors lib/brands.ts's nocase so dedupe/exact-match agree everywhere. */
const nocase = (s: string) => s.trim().normalize("NFD").replace(/[̀-ͯ]/g, "").toLowerCase();
...
export function BrandPickerSheet({
  ...
  // Enter/Return: same outcome as tapping the leading row — the add-row's create-and-pick
  // when it's showing, otherwise the top-ranked suggestion (the exact-ish match itself).
  const onSubmitEditing = () => {
    if (!q) return;
    if (!exactExists) { addAndPick(); return; }
    if (suggestions[0]) pick(suggestions[0].name);
  };

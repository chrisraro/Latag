...
3. **Phase C — Mobile licensing:** Supabase sign-in in app (Welcome/OTP screens already designed), license receipt caching, counter reconciliation, telemetry sync. **C SHIPPED 2026-07-14** (email-link sign-in via latag:// deep link + code entry ready for SMTP; license fetch + offline receipt cache; Settings; onboarding). Telemetry sync remains Phase D. RESOLVED 2026-07-27: custom SMTP (Resend) puts the 6-digit code in the email, so sign-in works from any device. The magic link remains same-device only (PKCE) but is now the secondary path.

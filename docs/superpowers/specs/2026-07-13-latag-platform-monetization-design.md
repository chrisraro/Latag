...
3. **Phase C — Mobile licensing:** Supabase sign-in in app (Welcome/OTP screens already designed), license receipt caching, counter reconciliation, telemetry sync. **C SHIPPED 2026-07-14** (email-link sign-in via latag:// deep link + code entry ready for SMTP; license fetch + offline receipt cache; Settings; onboarding). Telemetry sync remains Phase D. Known limitation until SMTP: the email link must be opened on the same device (PKCE); opening it elsewhere invalidates the pending code — Resend recovers.

...
- **IG share deviation (as built)**: no share-intent module is installed on the v1.1.0 runway (expo-sharing would need a native rebuild), so Share to IG = save photos to the session album → copy caption to clipboard → open the Instagram app (web fallback) — the saved photos are first in IG's recents picker. A future runway may add expo-sharing for a true multi-image share intent.

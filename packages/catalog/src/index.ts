import { formatInches } from "@latag/format";
 ...
 * `Pick<Item, "department" | SpecKey | "sizeNote">` from the E1 schema.
 */
...
/** Web landing page display data — keeps the marketing copy in sync with the catalog. */
export const DEPARTMENTS_DISPLAY: { name: string; specs: string; types: string }[] = [
...
// ---------------------------------------------------------------------------
// Spec parsing (for restore — reverse of specRowsFor)
// ---------------------------------------------------------------------------

/** Map from spec display label back to the local column key. */
export const SPEC_LABEL_TO_KEY: Record<string, SpecKey> = {
  "Pit-to-pit": "ptpInches",
  "Length": "lengthInches",
  "Sleeve": "sleeveInches",
  "Waist": "waistInches",
  "Inseam": "inseamInches",
  "Rise": "riseInches",
  "Leg opening": "legOpeningInches",
  "US size": "shoeSizeUs",
  "Insole": "insoleCm",
  "Width": "widthInches",
  "Height": "heightInches",
  "Depth": "depthInches",
  "Strap drop": "strapDropInches",
};

/**
 * Parse a value string like `21"` or `US 9.5` or `25.5 cm` back to a number.
 * Returns null if unparseable.
 */
export function parseSpecValue(value: string): number | null {
  ...
  if (trimmed.startsWith("US ")) {
    const n = parseFloat(trimmed.slice(3));
    return Number.isFinite(n) ? n : null;
  }
  // "25.5 cm" → 25.5
